...
The main design principle is simple YAML. A field should appear in YAML
...
Most projects should use bundled package assets. In that workflow, the
update path is simply:
...
After the package is updated, documents using
[`docdesigner::pdf`](https://jncohen.github.io/docdesigner/reference/pdf.md),
[`docdesigner::html`](https://jncohen.github.io/docdesigner/reference/html.md),
or
[`docdesigner::snapshot`](https://jncohen.github.io/docdesigner/reference/snapshot.md)
use the updated bundled assets automatically.

If a project keeps editable local copies, refresh those copies from the
installed package:
...
Snapshot YAML unless you are deliberately testing the template
internals.
...
Twelve styles ship in two sets: `academic` (`minimal`, `nature`,
`methods`, `demography`, `ssrn`, `sociology`, `ajs`) and `public`
(`policy`, `government`, `economist`, `atlantic`, `humanities`).

Per-style details — body font, accent colour, section numbering, column
count — are **not** listed here on purpose. A hand-written table of them
lived at this spot and silently went stale: after the rebuilt styles
were promoted on 2026-08-26, every accent colour in it was wrong. The
Style Gallery vignette builds the same table from the installed styles,
so it cannot drift:
...
A style is a folder with a `format.yml` file of design tokens. Built-in
styles ship inside two sets, `academic` and `public`:
...
Tokens are written as flat dotted keys — one `key: value` per line. Only
the identity keys (`id`, `label`, `description`) and the tokens that
differ from the defaults need to appear:
...
lists every available token with its type, allowed values, default, and
implementation status. `STYLE-SPEC.md` documents the whole vocabulary.
