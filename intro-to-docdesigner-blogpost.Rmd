...
abstract: |
  This post explains what the docdesigner TeX template does and how it fits into an
...
2. Run `docdesigner::docdesigner_use()` in a project folder.
3. Write an R Markdown file.
4. Add the docdesigner template settings in the YAML header.
5. Click **Knit** in RStudio.

The helper function copies the files your project needs:

```r
docdesigner::docdesigner_use()
```
...
| Fontset | Best use |
|---|---|
| `docdesigner` | Household Finance Lab reports and public-facing documents |
...
For that workflow, use `docdesigner::blogpost_pdf`:
...
For Snapshots, use `docdesigner::snapshot_pdf` and add `snapshot: true`:
