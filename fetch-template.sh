...
# fetch-template.sh
#
# Pulls the latest docdesignertemplate.tex, default.csl, style manifests, and
# font files from the docdesigner GitHub repository into a local managed
# directory.  Run whenever you want to update to the current version.
#
# Usage:
#   bash fetch-template.sh                        # update ~/Templates/docdesigner/
#   docdesigner_template_DIR=/custom/path bash fetch-template.sh
#   docdesigner_BRANCH=v1.0.0 bash fetch-template.sh      # pin to a specific tag
#
...
REPO="https://raw.githubusercontent.com/jncohen/docdesigner"
BRANCH="${docdesigner_BRANCH:-main}"
BASE="$REPO/$BRANCH"
DEST="${docdesigner_template_DIR:-$HOME/Templates/docdesigner}"

echo "Fetching docdesigner ($BRANCH) to: $DEST"
mkdir -p "$DEST/fonts"
...
# --- Core files ---
fetch "$BASE/inst/templates/docdesignertemplate.tex"  "$DEST/docdesignertemplate.tex"
fetch "$BASE/inst/csl/default.csl"            "$DEST/default.csl"

# --- Style manifests ---
echo "Fetching styles..."
for s in atlantic demography economist government humanities methods minimal nature policy ssrn; do
  mkdir -p "$DEST/styles/$s"
  fetch "$BASE/inst/styles/$s/style.yml" "$DEST/styles/$s/style.yml"
done
